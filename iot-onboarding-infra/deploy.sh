...
then
    ...
    cdk --app bin/iot-onboarding-infra.js synth -c envName=$env -c artifactBucket=$bucket > iot-onboarding-infra.yml
    ...
    cdk deploy IOTOnboardingInfraStack$env -c envName=$env -c artifactBucket=$bucket --format=json --require-approval never
    rc=$?
    if [ $rc -ne 0 ]; then
    ...
    iotSitewiseRole=$(aws cloudformation describe-stacks --stack-name RigadoInfraStack$env --query "Stacks[0].Outputs[?OutputKey=='iotSitewiseServiceRole'].OutputValue" --output text)
    ...
    echo "--------------------------------------------------------------------------------------------"
    echo "| Region                  |  $OUTRegion"
    echo "--------------------------------------------------------------------------------------------"
    echo "| Onboarding API          |  $apiEndpoint"
    echo "--------------------------------------------------------------------------------------------"
    echo "| Cognito User Pool ID    |  $userPoolId"
    echo "--------------------------------------------------------------------------------------------"
    echo "| Cognito Client ID       |  $clientId"
    echo "--------------------------------------------------------------------------------------------"
    echo "| Refresh Token           |  $refershToken"
    echo "--------------------------------------------------------------------------------------------"
    echo "| Password                |  $password"
    echo "--------------------------------------------------------------------------------------------"
    echo "| Token Endpoint          |  $tokenEnpoint"
    echo "--------------------------------------------------------------------------------------------"
    echo "| Glue DB Name            |  $glueDbName"
    echo "--------------------------------------------------------------------------------------------"    
    echo "| Athena Table Name       |  $athenaTableName"
    echo "--------------------------------------------------------------------------------------------"
    echo "| Iot Sitewise Role       |  $iotSitewiseRole"
    echo "--------------------------------------------------------------------------------------------"  
