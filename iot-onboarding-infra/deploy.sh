...
then
    ...
    cdk deploy IOTOnboardingInfraStack$env -c envName=$env -c artifactBucket=$bucket -c mqttTopic=$mqttTopic -format=json --require-approval never
    rc=$?
    if [ $rc -ne 0 ]; then
    ...
    | Region                  |  $OUTRegion\
    --------------------------------------------------------------------------------------------\
    | Onboarding API          |  $apiEndpoint\
    --------------------------------------------------------------------------------------------\
    | Cognito User Pool ID    |  $userPoolId\
    --------------------------------------------------------------------------------------------\
    | Cognito Client ID       |  $clientId\
    --------------------------------------------------------------------------------------------\
    | Refresh Token           |  $refershToken\
    --------------------------------------------------------------------------------------------\
    | Password                |  $password\
    --------------------------------------------------------------------------------------------\
    | Token Endpoint          |  $tokenEnpoint\
    --------------------------------------------------------------------------------------------\
    | Glue DB Name            |  $glueDbName\
    --------------------------------------------------------------------------------------------\    
    | Athena Table Name       |  $athenaTableName\
    --------------------------------------------------------------------------------------------\
    | Iot Sitewise Role       |  $iotSitewiseRole\
    --------------------------------------------------------------------------------------------"
    echo $summary
